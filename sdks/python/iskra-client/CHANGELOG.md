...
- **Requests:** `params` is added to a query already in the path; httpx replaced it (`get("/x?page=1", params={"size": 10})` sent only `size`). Empty `params` leave the path as is.
