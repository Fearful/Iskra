...
kernel.registerFeature(
    new AuthFeature({ secret: "a-contract-secret-with-enough-entropy-1f9c2e7b", baseURL: ORIGIN, basePath: "/api/sso", trustedOrigins: [ORIGIN] }),
);
...
const first: number[] = [];
for (let i = 0; i < 4; i++) first.push((await app.request("/api/sso/sign-in/email", signIn(), from("198.51.100.1"))).status);
const other = (await app.request("/api/sso/sign-in/email", signIn(), from("198.51.100.2"))).status;
const spoofed: number[] = [];
for (let i = 0; i < 4; i++) {
    spoofed.push((await app.request("/api/sso/sign-in/email", signIn({ "x-iskra-client-ip": `203.0.113.${i}` }), from("198.51.100.3"))).status);
}
