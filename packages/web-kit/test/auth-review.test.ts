...
async function productionKernel(authConfig: Record<string, unknown> = {}) {
    ...
    kernel.registerFeature(
        new AuthFeature({ secret: SECRET, basePath: '/api/sso', trustedOrigins: [ORIGIN], ...authConfig } as any),
    );
...
describe('AuthFeature request forwarding', () => {
    it('forwards a body-less POST (sign-out) without an empty body', async () => {
        ...
        kernel.registerFeature(db);
        kernel.registerFeature(
            new AuthFeature({ secret: SECRET, basePath: '/api/sso', baseURL: 'http://localhost:3000' } as any),
        );
        await kernel.initialize();
        (db.db as unknown as { $client: { exec(sql: string): void } }).$client.exec(DDL);
