...
function migrationsFolder(dialect: string, table: string, when: number): string {
    ...
    writeFileSync(
        ...
        JSON.stringify({
            version: '7',
            dialect,
            entries: [{ idx: 0, version: '7', when, tag: '0000_init', breakpoints: true }],
        }),
    );
...
describe('DbDriver.runMigrations applies generated migrations', () => {
        ...
        test.if(enabled)(label, async () => {
            ...
            try {
                ...
                for (const stmt of [
                    ...
                    `DELETE FROM ${journal} WHERE created_at = ${when}`,
                ]) {
    ...
    test('wraps a failing migration in MigrationError', async () => {
        ...
        try {
            await expect(driver.runMigrations(undefined, '/nonexistent/migrations')).rejects.toBeInstanceOf(
                MigrationError,
            );
        } finally {
            await app.stop();
