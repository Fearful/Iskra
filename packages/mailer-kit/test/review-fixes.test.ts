...
describe("SMTP", () => {
    function smtpAdapter() {
        ...
        adapter.transporter = ((nodemailer as any).default ?? nodemailer).createTransport({ streamTransport: true, buffer: true });
        const sent: string[] = [];
    ...
    it("forwards allowed headers and rejects others", async () => {
        ...
        await expect(adapter.send({ to: "u@example.com", subject: "s", text: "t", headers: { "X-Custom": "v" } })).rejects.toThrow(
            /not allowed/,
        );
    });
});
...
describe("SES", () => {
    function sesAdapter() {
        ...
        const adapter = new SesEmailAdapter(
            ...
            {
                client: { send: async (c: any) => { inputs.push(c.input); return { MessageId: "m" }; } },
    ...
    it("fails instead of silently dropping attachments or headers", async () => {
        ...
        await expect(
            adapter.send({ to: "u@example.com", subject: "s", text: "t", attachments: [{ filename: "f.pdf", content: new Uint8Array([1]) }] }),
    ...
    it("encodes a non-ASCII display name", async () => {
        ...
        expect(inputs[0].FromEmailAddress).toBe(`=?UTF-8?B?${Buffer.from("José Pérez").toString("base64")}?= <n@example.com>`);
    });
});
...
describe("formatAddress", () => {
    it("quotes special characters and rejects a malformed address", () => {
        ...
        expect(formatAddress({ name: "Line\r\nBcc: x@y.com", email: "a@example.com" })).toBe('"Line Bcc: x@y.com" <a@example.com>');
    ...
    it("is used by Mailgun", async () => {
        ...
        try {
            ...
            await mailgun.send({ from: { name: 'A" <x@evil.com>', email: "a@example.com" }, to: "u@example.com", subject: "s", text: "t" });
        } finally {
