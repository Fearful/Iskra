...
4. **`app.start()`** — Calls `init()` on every driver, then `start()` one at a time in registration order. If a driver's `init()` fails, the drivers whose `init()` ran so far (the failing one included; a driver without `init` was never initialized and is not stopped) are stopped in reverse order; if a driver fails to start, every driver is stopped in reverse order. In both cases OpenTelemetry is shut down, the error is rethrown, and a later `app.stop()` stops nothing more.
