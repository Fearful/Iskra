...
- Los adapters Redis y en memoria de `CacheFeature` se comportan igual (**breaking**): Redis guarda todo valor como JSON, tambien los strings, asi que `'123'` se lee como string y no como `123` (los strings crudos que guardaban versiones anteriores se siguen leyendo); un TTL fraccionario en segundos (`0.5`) funciona en Redis, que lo rechazaba; y `increment()` sobre una key inexistente o vencida la crea en 1 en memoria, como `INCR` de Redis (devolvia 0).
