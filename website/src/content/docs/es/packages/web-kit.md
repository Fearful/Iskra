...
- Una request por encima del limite recibe un 429 con `Retry-After`: los segundos hasta que se reinicia la ventana del cliente (al menos 1). Si el store no sabe cuando se reinicia (`store: 'cache'` despues del primer hit de la ventana), es `windowMs`, una cota superior. Un `handler` propio tambien lo envia si responde a traves de `c` (`c.json()`, `c.text()`). Los SDKs cliente lo leen en su excepcion de rate limit.
...
- Los adapters Redis y en memoria de `CacheFeature` se comportan igual (**breaking**): Redis guarda todo valor como JSON, tambien los strings, asi que `'123'` se lee como string y no como `123` (los strings crudos que guardaban versiones anteriores se siguen leyendo), y por eso un contador debe guardarse como numero (`set('k', 5)`) o crearse con `increment()` (despues de `set('k', '5')`, `increment()` falla en Redis); un TTL fraccionario en segundos (`0.5`) funciona en Redis, que lo rechazaba; y `increment()` sobre una key inexistente o vencida la crea en 1 en memoria, como `INCR` de Redis (devolvia 0).
...
- **Permisos (`PermissionsFeature`):** los permisos y roles de cada usuario se cachean (con `CacheFeature`) durante `cacheTTL` segundos, 60 por defecto (antes una hora): un rol que revocas sigue valiendo ese tiempo. Llama a `await kernel.getFeature('permissions')?.invalidate(userId)` despues de cambiar los roles o permisos de un usuario para descartar la copia cacheada; baja `cacheTTL`, o usa `cachePermissions: false`, para cambiar consultas a la base por una ventana mas corta. Corre despues de `auth`, `session` y `cache` sea cual sea el orden en que las registres; ninguna es obligatoria: sin una feature de auth o de sesion cada request es anonimo y recibe solo `anonymousPermissions`.
...
- **Uploads (`UploadFeature`):** con `exposeRoutes: true` es obligatorio `authorize(c, action, target?)` (`action`: `upload` | `list` | `download` | `delete`); usa `authorize: () => true` solo si las rutas deben ser publicas. El body se corta al superar `maxFileSize` (413) sin cargarlo entero en memoria, el nombre del archivo se sanea y los errores internos no se devuelven al cliente. `maxFileSize` mas 64 KiB de overhead multipart tiene que entrar en el `maxRequestBodySize` del Kernel (16 MiB por defecto): Bun rechaza bodies mas grandes antes de llegar a la ruta, asi que `initialize()` falla en vez de que el limite nunca se alcance sin aviso.
    ...
    - `c.get('upload').uploadFromRequest(request, field)` aplica las mismas reglas (**breaking**): corta el body al superar `maxFileSize` y rechaza una extension que `allowedExtensions` no permite, lanzando un `HttpError` (413 `File too large`, 400 `Invalid extension`, 400 si el campo no trae un archivo). Antes leia cualquier body entero y guardaba cualquier extension. Sigue el `maxFileSize` y los `allowedExtensions` de la feature, este o no activo `exposeRoutes`.
...
- Con `store: 'db'`, un error de la base al leer, escribir o borrar una sesion hace fallar la request (500, y no se envia cookie). Antes solo se logueaba: el cliente recibia una cookie de una sesion que nunca se guardo, y un logout fallido parecia exitoso. Una fila cuyos datos no son JSON valido se loguea y se trata como si no hubiera sesion.
