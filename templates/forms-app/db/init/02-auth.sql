-- Better Auth's tables for admin-api (matching @iskra-bun/auth-kit's pgSchema).
-- Postgres runs this on the first start of an empty data volume; admin-api's
-- `bun run create-admin` applies it too (IF NOT EXISTS), then creates an admin.
